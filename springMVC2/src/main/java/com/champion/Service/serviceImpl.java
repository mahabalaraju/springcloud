...
public class serviceImpl implements personService {
   ...
   Optional<person> per=  Optional.ofNullable(repo.findById(id)).orElseThrow(()->new personnotfoundexception("no person found with given id"+id));
     if(per.isPresent())
    		repo.deleteById(id);
	...
	public Optional<person> getbyid(int id) {
		return Optional.ofNullable(repo.findById(id)).orElseThrow(()->new personnotfoundexception("no person found with given id"+id));
